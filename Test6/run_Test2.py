...
for N in [200, 400, 1000, 5000, 10000, 20000, 60000]:
    t_data_HF, s_data_HF = create_dataset_HF(Test=False, N=N)
    ...
    HF_model = MF_KAN(layer_dims_nl, layer_dims_l, LF_model, params_LF, boundaries, scales, 
                      grid_vals, init_lr, 0)
    time = HF_model.train(num_epochs_HF, t_data_HF, s_data_HF)
    print(time)
    ...
    scipy.io.savemat("Test7_64_" + str(N) + "_loss.mat", 
                     {'train_losses':train_losses,
                     'eval_losses':eval_losses_HF}
                     , format='4')
    with open('MF_64_' + str(N) + '.pkl', 'wb') as f:
        pickle.dump(params_HF, f)
