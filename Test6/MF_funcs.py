...
class MF_KAN:
    def __init__(self, layer_dims_nl, layer_dims_l, SF_model, SF_variables, boundaries, scales, 
        ...
        variables2 = self.modelHF_nl.init(key, jnp.ones([1, 785]))
        variable_params = [variables2['params']]
        variables = [variables2]
        
        key, subkey = random.split(key)
        variables2 = self.modelHF_lin.init(key, jnp.ones([1, 1]))
    ...
    def op_net(self, params, xHF, state):
        ...
        invals = jnp.hstack([xHF, preds_LF_on_HF.reshape(-1, 1)])
        ...
        state_cur = state[1]
        variables1 = {'params' : param_cur, 'state' : state_cur}
        preds_lin, spl_regs3 = self.modelHF_lin.apply(variables1, preds_LF_on_HF.reshape(-1, 1))
    ...
    def simple_out_fn(self, xHF, varis):
        ...
        preds_HF, _, _ = vmap(self.op_net, (None, 0, None))(params, xHF.reshape(-1, 1, 784), states)
        preds_HF = preds_HF.reshape(-1, 1)
        return preds_HF
    ...
    def op_both(self, params, xHF, state):
        ...
        state_cur = state[1]
        variables1 = {'params' : param_cur, 'state' : state_cur}
        preds_lin, spl_regs3 = self.modelHF_lin.apply(variables1, preds_LF_on_HF)
    ...
    def both_out(self, xHF, varis):
        ...
        preds_nonlin, preds_lin, preds_LF_on_HF = vmap(self.op_both, (None, 0, None))(params, xHF.reshape(-1, 1, 1), states)
        return preds_nonlin, preds_lin, alpha, preds_LF_on_HF


    def op_net2(self, params, xHF, state):
        xHF = xHF.reshape(-1, 1)
        preds_LF_on_HF = self.modelLF.simple_out_fn(xHF, self.SF_variables)
    ...
    def simple_loss_fn(self, params, x, y, state):
        predsHF, spl_regs2, spl_regs3 = self.op_net(params, x, state)
        loss_pred = (jnp.mean((predsHF.flatten()-y.flatten())**2))
        ...
        loss_pred = loss_pred + 0.01*alpha[0]**4 \
                    + self.norm_weight*loss_reg
        return loss_pred
    
    
    @partial(jit, static_argnums=(0,))
    def train_step(self, params, states, opt_state, x,y):

        loss, grads = jax.value_and_grad(self.simple_loss_fn)(params,  x, y,  states)
    ...
    def train(self, num_epochs, x, y):
     ...
     #       if epoch in []:
                ...
                idx = random.choice(key, Nin, [l,], replace=False)
                noise = 0.2*random.uniform(key, [l, 785])

                preds_LF_on_HF = self.modelLF.simple_out_fn(x[idx, :], self.SF_variables)

                invals = jnp.hstack([x[idx, :], preds_LF_on_HF.reshape(-1, 1)])+noise
                variables = self.variables[0]
                updated_variables1 = self.modelHF_nl.apply(variables, invals, G_new, 
                ...
                invals = jnp.asarray([0, 2]).reshape(-1, 1)
            ...
            key, subkey = random.split(key)
            idx = random.choice(key, Nin, [64,], replace=False)


            self.variables, self.opt_state, loss= self.train_step(params,
                                                                  states, 
                                                                  self.opt_state,
                                                                  x[idx, :], y[idx, :])
