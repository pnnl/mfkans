...
class SF_KAN:
    def __init__(self, layer_dims, boundaries, scales, grid_vals, init_lr, norm_weight=0): 
        ...
        self.variables = self.modelLF.init(key, jnp.ones([1, 784]))
    ...
    def simple_out_fn(self, xLF, varis ):
        params = varis['params'] 
        state = varis['state']
        preds_HF, _ = vmap(self.op_net, (None, 0, None))(params, xLF.reshape(-1, 1, 784), state)
    ...
    def train(self, num_epochs, t_data, s_data):
        ...
        #    if epoch in []:
                ...
                noise = 0.2*random.uniform(key, [l, 784])

                updated_variables1 = self.modelLF.apply(self.variables, t_data[idx, :]+noise, G_new, 
