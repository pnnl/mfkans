import torch
import numpy as np
import sys
sys.path.insert(0, '../')

import jax
import jax.numpy as jnp
import optax
from flax import linen as nn
from jax import random, grad, vmap, jit

from KAN import KAN
from jax import vmap
import math
from functools import partial

from tqdm import trange, tqdm
import scipy.io
from jax.nn import relu, elu, selu, swish
import itertools
from jax.example_libraries import optimizers


def DNN(branch_layers, activation=jnp.tanh):

    def xavier_init_j(key, d_in, d_out):
        glorot_stddev = 1. / np.sqrt((d_in + d_out) / 2.)
        W = glorot_stddev * random.normal(key, (d_in, d_out))
        b = np.zeros(d_out)
        return W, b
    def init(rng_key1):
        def init_layer(key, d_in, d_out):
            k1, k2 = random.split(key)
            W, b = xavier_init_j(k1, d_in, d_out)
            return W, b
        key1, *keys1 = random.split(rng_key1, len(branch_layers))
        branch_params = list(map(init_layer, keys1, branch_layers[:-1], branch_layers[1:]))
        return branch_params
        
    def apply(params, u):
      #  print(u.shape)
        for k in range(len(branch_layers)-2):
            W_b, b_b = params[k]
            
            u = activation(jnp.dot(u, W_b) + b_b)

        W_b, b_b = params[-1]
        u = jnp.dot(u, W_b) + b_b
      #  print(u.shape)

        return u

    return init, apply

def lin_DNN(branch_layers, activation=swish):

    def xavier_init_j(key, d_in, d_out):
        glorot_stddev = 1. / np.sqrt((d_in + d_out) / 2.)
        W = glorot_stddev * random.normal(key, (d_in, d_out))
        b = np.zeros(d_out)
        return W, b
    def init(rng_key1):
        def init_layer(key, d_in, d_out):
            k1, k2 = random.split(key)
            W, b = xavier_init_j(k1, d_in, d_out)
            return W, b
        key1, *keys1 = random.split(rng_key1, len(branch_layers))
        branch_params = list(map(init_layer, keys1, branch_layers[:-1], branch_layers[1:]))
        return branch_params
        
    def apply(params, u):
      #  print(u.shape)
        for k in range(len(branch_layers)-2):
            W_b, b_b = params[k]
            
            u = (jnp.dot(u, W_b) + b_b)

        W_b, b_b = params[-1]
        u = jnp.dot(u, W_b) + b_b
      #  print(u.shape)

        return u

    return init, apply


class MF_KAN:
    def __init__(self, layer_dims_nl, layer_dims_l, SF_model, SF_variables, init_lr, norm_weight): 
        
        self.SF_variables = SF_variables
        self.modelLF = SF_model
        
        self.init_nl, self.apply_nl = DNN(layer_dims_nl)
        self.init_l, self.apply_l = lin_DNN(layer_dims_l)
        
        
        params_nl = self.init_nl(random.PRNGKey(12))
        params_l = self.init_l(random.PRNGKey(12))
        
        self.itercount = itertools.count()
        
        

        
        key = jax.random.PRNGKey(10)


        alpha = np.abs(random.uniform(key, shape=[1]))
        alpha = jnp.asarray([.2])

        params = params_nl, params_l, alpha
        
        self.opt_init, \
        self.opt_update, \
        self.get_params = optimizers.adam(init_lr)
 
        self.opt_state = self.opt_init(params)

        self.train_losses =[]
        self.eval_losses_HF = []
        
        self.norm_weight = norm_weight


    
    def op_net(self, params, xHF):
        params_nl, params_l, alpha = params
        preds_LF_on_HF = self.modelLF.simple_out_fn(xHF, self.SF_variables)
    
        invals = jnp.hstack([xHF, preds_LF_on_HF])
    

        preds_nonlin = self.apply_nl(params_nl, invals)
        
        preds_lin = self.apply_l(params_l, invals)
        

        preds2 = jnp.abs(alpha)*preds_nonlin+(1-jnp.abs(alpha))*preds_lin
    
        return preds2
    
    
    def op_net2(self, params, xHF):
        params_nl, params_l, alpha = params
        preds_LF_on_HF = self.modelLF.simple_out_fn(xHF, self.SF_variables)

        invals = jnp.hstack([xHF.reshape(-1), preds_LF_on_HF.reshape(-1)])
    

        preds_nonlin = self.apply_nl(params_nl, invals)
        
        preds_lin = self.apply_l(params_l, invals)
        

        preds2 = jnp.abs(alpha)*preds_nonlin+(1-jnp.abs(alpha))*preds_lin
        return preds2[0]
        
    
    def physics_net(self, params, x):
        xval = x[0]

        u_xx = grad(grad(self.op_net2, argnums=1), argnums=1)(params, xval)

        return u_xx
    
    
    @partial(jit, static_argnums=(0,))
    def residual_loss(self, params, x, f):
        res =  vmap(self.physics_net, (None, 0))(params, x)
        loss = (jnp.mean((res.flatten()-f.flatten())**2))
    
        return loss
        
    def simple_out_fn(self, xHF, params):
        preds_HF = vmap(self.op_net, (None, 0))(params, xHF)
        return preds_HF
    
    def op_both(self, params, xHF):
        params_nl, params_l, alpha = params
        preds_LF_on_HF = self.modelLF.simple_out_fn(xHF, self.SF_variables)
    
        invals = jnp.hstack([xHF, preds_LF_on_HF])
    

        preds_nonlin = self.apply_nl(params_nl, invals)
        
        preds_lin = self.apply_l(params_l, invals)
        

    
        return preds_nonlin, preds_lin, preds_LF_on_HF
    
    
    def both_out(self, xHF, params):
        params_nl, params_l, alpha = params


        preds_nonlin, preds_lin, preds_LF_on_HF = vmap(self.op_both, (None, 0))(params, xHF)
        return preds_nonlin, preds_lin, alpha, preds_LF_on_HF




    
    def simple_loss_fn(self, params,t_data, f_data, t_data_bc, s_data_bc):
        predsHF= self.op_net(params, t_data_bc)
    
        params_nl, params_l, alpha = params


        r_loss = self.residual_loss(params, t_data, f_data)

        loss_pred = r_loss + (jnp.mean((predsHF.flatten()-s_data_bc.flatten())**2)) + 100000*alpha[0]**4 
        return loss_pred




    
    @partial(jit, static_argnums=(0,))
    def train_step(self,i, opt_state, t_data, f_data, t_data_bc, s_data_bc):

        params = self.get_params(opt_state)

       # loss, grads = jax.value_and_grad(self.simple_loss_fn)(params,  xLF, yLF,  states)

        g = grad(self.simple_loss_fn)(params, t_data, f_data, t_data_bc, s_data_bc)
        return self.opt_update(i, g, opt_state)


    
    def train(self, num_epochs, t_data, f_data, t_data_bc, s_data_bc):
        pbar = trange(num_epochs)
        key = jax.random.PRNGKey(10)

        for epoch in pbar:
            key, subkey = random.split(key)

            inds = random.choice(key, len(t_data), (400,), replace=False)

            self.opt_state = self.train_step(next(self.itercount), self.opt_state, 
                                     t_data[inds, :], 
                                     f_data[inds], 
                                     t_data_bc, 
                                     s_data_bc)

            
            if epoch % 100 == 0:
                params = self.get_params(self.opt_state)
                loss_value = self.simple_loss_fn(params, t_data, f_data, t_data_bc, s_data_bc)
                params_nl, params_l, alpha = params

              #  preds_HF = self.simple_out_fn(t_data, params)
                loss_HF = 0

                self.train_losses.append(loss_value)
                pbar.set_postfix({'Loss': "{0:.6f}".format(loss_value),
                                  'alpha': "{0:.6f}".format(alpha[0])})
                elapsed = pbar.format_dict['elapsed']
        print(alpha)

        return elapsed

                
