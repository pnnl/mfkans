...
class MF_KAN:
    def __init__(self, layer_dims_nl, layer_dims_l, SF_model, SF_variables, boundaries, scales, 
        ...
        variables2 = self.modelHF_nl.init(key, jnp.ones([1, 2]))
        variable_params = [variables2['params']]
        variables = [variables2]
        
        key, subkey = random.split(key)
        variables2 = self.modelHF_lin.init(key, jnp.ones([1, 2]))
    ...
    def physics_net(self, params, x, state):
        xval = x[0]

        u_xx = grad(grad(self.op_net2, argnums=1), argnums=1)(params, xval, state)

        return u_xx
    
    
    @partial(jit, static_argnums=(0,))
    def residual_loss(self, params, x, f, state):
        res =  vmap(self.physics_net, (None, 0, None))(params, x, state)
    ...
    def simple_loss_fn(self, params,t_data, f_data, t_data_bc, s_data_bc, state):
        predsHF, spl_regs2, spl_regs3 = self.op_net(params, t_data_bc, state)
        ...
        r_loss = self.residual_loss(params, t_data, f_data, state)
    ...
    @partial(jit, static_argnums=(0,))
    def train_step(self, params, states, opt_state, t_data, f_data, t_data_bc, s_data_bc):

        loss, grads = jax.value_and_grad(self.simple_loss_fn)(params,  t_data, f_data, t_data_bc, s_data_bc,  states)
    ...
    def train(self, num_epochs, t_data, f_data, t_data_bc, s_data_bc, s_data):
      ...
      #          invals = jnp.asarray([[0, 1],[0, 1],[0, 1],[0, 1], [-2, 2]])
            ...
            self.variables, self.opt_state, loss= self.train_step(params, states, self.opt_state, 
                                                                  t_data[inds, :], f_data[inds],  t_data_bc, s_data_bc)
