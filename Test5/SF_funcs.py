...
class SF_KAN:
    ...
    def op_net2(self, params, xLF, state):
        param_cur = params
        state_cur = state
        xLF = xLF.reshape(-1, 1)
    ...
    def simple_loss_fn(self, params, xLF, fLF, x_BC, s_BC, state):
        preds_BC, spl_regs  = self.op_net(params, x_BC, state)
        loss_pred = (jnp.mean((preds_BC.flatten()-s_BC.flatten())**2))
            
        r_loss = self.residual_loss(params, xLF, fLF, state)
        ...
        loss = r_loss + 100*loss_pred + self.norm_weight*loss_reg
        return loss
    
    @partial(jit, static_argnums=(0,))
    def train_step(self,  params, states, opt_state, xLF, fLF, x_BC, s_BC):

        loss, grads = jax.value_and_grad(self.simple_loss_fn)(params,  xLF, fLF, x_BC, s_BC,  states)
    ...
    def train(self, num_epochs, t_data, f_data, t_data_bc, s_data_bc):
        ...
        for epoch in pbar:
            ...
            self.variables, self.opt_state, loss= self.train_step(params, states, 
                                                                  self.opt_state, 
                                                                  t_data[inds, :], 
                                                                  f_data[inds], 
                                                                  t_data_bc, 
