...
class MF_KAN:
    def __init__(self, layer_dims_nl, layer_dims_l,  boundaries, scales, 
        ...
        variables2 = self.modelHF_nl.init(key, jnp.ones([1, 3]))
        variable_params = [variables2['params']]
        variables = [variables2]
        
        key, subkey = random.split(key)
        variables2 = self.modelHF_lin.init(key, jnp.ones([1, 3]))
...
#        preds_LF_on_HF = self.modelLF.simple_out_fn(xHF, self.SF_variables)
 ...
 #       invals = jnp.hstack([xHF[:, 0:2], preds_LF_on_HF])
        invals = xHF
        param_cur = params[0]
        state_cur = state[0]
...
#        loss_pred = (jnp.mean((predsHF.flatten()-yHF.flatten())**2*(1+yHF.flatten())**2)) + .1*alpha[0]**4 + self.norm_weight*loss_reg                        
        loss_pred = (jnp.mean((predsHF.flatten()-yHF.flatten())**2)) + .1*alpha[0]**4 + self.norm_weight*loss_reg                        
        return loss_pred
    ...
    def train(self, num_epochs, t_data, s_data):
        ...
        for epoch in pbar:
            ...
            if epoch in self.grid_upds.keys():
                ...
                inds = random.choice(key, len(t_data), (10000,), replace=False)

#                preds_LF_on_HF = self.modelLF.simple_out_fn(t_data[inds, :], self.SF_variables)
 
 #               invals = jnp.hstack([t_data[inds, 0:2], preds_LF_on_HF])
                invals = t_data[inds, :]
