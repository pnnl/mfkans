...
class SF_KAN:
    def __init__(self, layer_dims, boundaries, scales, grid_vals, init_lr, norm_weight=0): 
        ...
        self.variables = self.modelLF.init(key, jnp.ones([1, 3]))
    ...
    def train(self, num_epochs, t_data, s_data):
        ...
        inds_list = np.arange(len(t_data))
        ...
        for epoch in pbar:
            ...
            if epoch in self.grid_upds.keys():
                ...
                inds = random.choice(key, len(t_data), (2000,), replace=False)

                # Perform the update
                updated_variables1 = self.modelLF.apply(self.variables, t_data[inds, :], G_new, 
          ...
          # inds = random.choice(key, 20000, (1000,), replace=False)
          
         #   inds = get_random_key(inds_list)
